...
class logisticRegression(torch.nn.Module):
    ...
    def forward(self, x, y):
        out = torch.sigmoid(torch.squeeze(linear(x, self.weights)))
...
def main():
    ...
    for i in range(num_elements):
        for j in range(dim):
            r1, r2 = random.uniform(0, 0.1), random.uniform(0, 0.1)/dim
            input[i][j] = (int)((i-num_elements/2)*r1 + j*r2)%10 if j%2 == 0 else (int)(-1*((i-num_elements/2)*r1 + j*r2))%10
        input[i][dim] = 1 if groud_truth.dot(input[i][:-1]) > 0 else 0
    ...
    model = logisticRegression(dim, init_weights)
